...
const services = [
  { name: 'Videa interiéru a exteriéru',       bg: 'assets/Ikony/rp5.webp' },
  { name: 'Fotografické služby',                bg: 'assets/Ikony/rp4.webp' },
  { name: 'Kampaně na sociálních sítích',       bg: 'assets/Ikony/rp2.webp' },
  { name: 'Vlastní web prodávané nemovitosti',  bg: 'assets/Ikony/rp9.webp' },
  { name: 'Vizualizace 2D a 3D půdorysů',       bg: 'assets/Ikony/rp8.webp' },
  { name: 'Virtuální homestaging',              bg: 'assets/Ikony/rp1.webp' },
  { name: 'Matterport prohlídky',               bg: 'assets/Ikony/rp6.webp' },
  { name: 'Právní a smluvní servis',            bg: 'assets/Ikony/rp3.webp' },
  { name: 'Stanovení ceny nemovitosti',         bg: 'assets/Ikony/rp7.webp' },
];
...
const pudorysyImages = [
  ...
  { src: 'assets/pudorysy/RADEGAST_INDEX_1080x1350-1-re7jpd8882f7n25qq39dbr4p6esldn2wvow7k4urls.png', alt: 'Vizualizace 3D půdorysu nemovitosti' },
  { src: 'assets/pudorysy/s-nabytkem-otoceno-reaqq0zodoo9k1mb6ew3ndeqgg0a5q7pu20mtzgfuo.png', alt: 'Půdorys s rozmístěným nábytkem' },
  { src: 'assets/pudorysy/Screenshot-2025-10-29-155607-1-re7jodid0d2bdvlmintjp056lron95542s3rbmbq68.png', alt: 'Půdorys nemovitosti – 3D pohled' },
  ...
  { src: 'assets/pudorysy/Screenshot-2025-10-29-155731-reaqr5eqjk7lf9zlmed24l7k00h2cpo6bm2igvsif4.jpg', alt: 'Půdorys nemovitosti – 2D plán' },
];
...
function createSlider({ trackId, prevId, nextId, dotsId, images, lightbox = true }) {
  ...
  function updateDots(idx) {
    const realIdx = ((idx % images.length) + images.length) % images.length;
    dotsEl.querySelectorAll('.slider-dot').forEach((d, i) => d.classList.toggle('active', i === realIdx));
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (document.getElementById('reviews-prev')) {
    ...
    if (reviewsViewport) {
      let swipeStartX = 0;
      reviewsViewport.addEventListener('touchstart', e => { swipeStartX = e.touches[0].clientX; }, { passive: true });
      reviewsViewport.addEventListener('touchend', e => {
        const diff = swipeStartX - e.changedTouches[0].clientX;
        if (Math.abs(diff) > 40) goToReview(reviewsOffset + (diff > 0 ? 1 : -1));
      }, { passive: true });
    }
  }
